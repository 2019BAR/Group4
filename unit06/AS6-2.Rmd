...
```{r}
par(cex = 0.7)
plot(0, 0, xlim = c(0, 6), ylim = c(0, 1), xlab = "噴發時間(分鐘)", ylab = "密度 or (累計)機率", main = "分布、機率與密度")
...
```{r}
i1=seq(1.6,5.1-1/6,1/6)
i2=seq(1.6+1/6,5.1,1/6)
p2=data.frame(i1,i2)
for(i in 1:21){
...
```{r}
70 * sum(p2[c(2, 3, 16, 17, 18, 19),3]) - 30 * (1 - sum(p2[c(2, 3, 16, 17, 18, 19),3])) 
#ANS:賭金30，期望利潤20.731
```
...
ANS:不一樣，最大期望報酬率還要考量投入成本，已讓每一分成本達到最大利用價值，而最大期望報酬只求報酬最大
&emsp;■ 你認為哪一個策略目標比較好呢？ 為什麼？<br>
ANS:最大期望報酬率，因為有考量到成本
<br>
</p class="qiz">
