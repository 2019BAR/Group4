...
```{r}
set.seed(88)
#set.seed()主要目的是讓模擬能夠可重複出現，因為很多時候我們需要取隨機數，但這段code再跑一次後結果就不一樣了，如果需要重複出現同樣的模擬結果的話，就可以用set.seed()。

split = sample.split(D$PoorCare, SplitRatio = 0.75)  #split vector，SplitRatio要切出來的比率。
table(split) %>% prop.table()　#1表左右相加為一
```

+ 目標變數與分割向量的列聯表
```{r}
table(D$PoorCare, split) %>% prop.table(2) #split : True 訓練集 False 測試集 # 2表上下相加為一
```
...
#PoorCare ~ OfficeVisits + Narcotics 表選取的欄位，TR 表選取資料表，family表需要哪種的機率分配。
summary(glm1)
```
<br><hr>
...
predict(glm1, df, type="response") #type="response" 表需要的是glm1為1的機率，非勝算(odds)
```
...
```{r}
exp(0.0821)  #將officevisits的係數取指數得勝率效果

```


```{r}
exp(0.0763)  #將narcotics的係數取指數得勝率效果
...
#officevisits的機率效果
df = data.frame(OfficeVisits = 7, Narcotics = 0)   
p1 = predict(glm1, df, type="response")
df = data.frame(OfficeVisits = 8, Narcotics = 0)   
p2 = predict(glm1, df, type="response")
p2 - p1
```
officevisits每增加一次，機率約增加0.8%

```{r}
#narcotics的機率效果
df = data.frame(OfficeVisits = 7, Narcotics = 0)   
p3 = predict(glm1, df, type="response")
df = data.frame(OfficeVisits = 7, Narcotics = 1)   
p4 = predict(glm1, df, type="response")
p4 - p3
```
narcotics每增加一次，機率約增加0.78%<br>
...
p4 - p3
```
narcotics每增加一次，機率約增加1.5%<br>

&emsp; ■ 比較以上兩個題目的答案，我們可以觀察到什麼？<br><br>

勝率效果是固定不會變的，而機率效果是不固定的，必須根據前一個機率去推論下一個
</p class="qiz">
